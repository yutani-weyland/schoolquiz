...
CREATE TABLE categories (
  ...
  type TEXT NOT NULL DEFAULT 'standard' CHECK (type IN ('standard', 'quick-fire')),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
BEGIN
  ...
  -- Enforce 5 categories and 25 questions
  IF category_count != 5 THEN
    RAISE EXCEPTION 'Quiz must have exactly 5 categories, found %', category_count;
  END IF;
  
  IF total_questions != 25 THEN
    RAISE EXCEPTION 'Quiz must have exactly 25 questions, found %', total_questions;
  END IF;
