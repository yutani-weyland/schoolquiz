...
export default function CreateQuiz() {
  ...
  const addRound = (categoryName: string) => {
    const colorIndex = quiz.rounds.length % roundColors.length;
    const newRound: QuizRound = {
      id: `round-${Date.now()}`,
      category: categoryName,
      title: `${categoryName} Round`,
      blurb: '',
    ...
    // Set the new round as active tab
    setActiveRoundTab(newRound.id);
  };
  ...
  const importQuestion = (roundId: string, questionData: any) => {
    setQuiz(prev => ({
      ...prev,
      rounds: prev.rounds.map(round =>
        round.id === roundId && round.questions.length < 5
          ? {
              ...round,
              questions: [
                ...round.questions,
  ...
  const saveQuiz = (asDraft = false, scheduleForLater = false) => {
    ...
    if (!asDraft && !scheduleForLater) {
      if (quiz.rounds.length !== 5) {
        alert('Please create exactly 5 rounds.');
        return;
      }
      if (!quiz.rounds.every(round => round.questions.length === 5)) {
        alert('Each round must have exactly 5 questions.');
        return;
      }
    }
                 ...
                 <div className="mb-8">
                   <div className="flex items-center justify-between mb-6">
                     <div>
                       ...
                       <p className="text-gray-600 dark:text-gray-400">Create a new quiz with 5 rounds of 5 questions each</p>
                     </div>
                     <div className="flex items-center gap-3">
                       <button
                       ...
                       {quiz.rounds.map((round, index) => {
                         ...
                         return (
                           <div key={round.id} className="flex items-center gap-2 relative">
                             ...
                             >
                        {round.category} ({round.questions.length}/5)
                  ...
                  return (
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-xs px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-full">
                            {activeRound.questions.length}/5 questions
                          </span>
                        </div>
                               <button
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {activeRound.questions.length < 5 && (
                              <button
                            ...
                            {activeRound.questions.length < 5 && (
                                   <button
                                     ...
                                     className={`px-3 py-1 text-sm text-white rounded-full transition-colors font-medium flex items-center gap-1 ${
                                       buttonColors[getRoundIndex(activeRound.id) % buttonColors.length]
                                     }`}
                                   >
                              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" />
                              </svg>
                              Add Question ({5 - activeRound.questions.length} remaining)
                            </button>
                            )}
                          </div>
                        </div>
                   ...
                   <aside className="hidden lg:block space-y-4">
                       ...
                       <div className="space-y-3">
                         ...
                         <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                           <div 
                             ...
                             style={{ width: `${(quiz.rounds.length / 5) * 100}%` }}
                           ></div>
                         </div>
                         <div className="flex justify-between items-center">
                           <span className="text-sm text-gray-600 dark:text-gray-400">Questions</span>
                           <span className="text-sm font-medium text-gray-900 dark:text-white">
                             {quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0)}/25
                           </span>
                         </div>
                         <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                           <div 
                             className="bg-green-600 h-2 rounded-full transition-all duration-300"
                             style={{ width: `${(quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0) / 25) * 100}%` }}
                           ></div>
                         </div>
                         <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
                           <div className="text-xs text-gray-500 dark:text-gray-400">
                             Ready for publish: {Math.round((quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0) / 25) * 100)}%
                           </div>
                         </div>
                       </div>
                     </div>
                     ...
                     <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700">
                       ...
                       <div className="space-y-3">
                         ...
                         <div className="flex items-center justify-between">
                           <span className="text-sm text-gray-600 dark:text-gray-400">Avg Questions/Round</span>
                           <span className="text-sm font-medium text-gray-900 dark:text-white">
                             {quiz.rounds.length > 0 ? Math.round(quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0) / quiz.rounds.length) : 0}
                           </span>
                         </div>
                         <div className="flex items-center justify-between">
                           <span className="text-sm text-gray-600 dark:text-gray-400">Completion Status</span>
                           <span className={`text-sm font-medium ${
                             quiz.rounds.length === 5 && quiz.rounds.every(round => round.questions.length === 5)
                               ? 'text-green-600 dark:text-green-400'
                               : 'text-orange-600 dark:text-orange-400'
                           }`}>
                             {quiz.rounds.length === 5 && quiz.rounds.every(round => round.questions.length === 5) ? 'Complete' : 'In Progress'}
                           </span>
                         </div>
                       </div>
                     </div>
               ...
               <div className="max-w-full mx-auto px-6 flex justify-between items-center">
                 <div className="flex items-center space-x-4">
                   <div className="text-sm text-gray-600 dark:text-gray-400">
                     Saved 2 min ago • {Math.round((quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0) / 25) * 100)}% complete
                   </div>
                   <div className="flex items-center space-x-2">
                     ...
                     <span className="text-xs text-gray-500 dark:text-gray-400">
                       {quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0)}/25 questions
                     </span>
                   </div>
                 </div>
                 <div className="flex gap-3">
                   ...
                   <button
                     ...
                     disabled={quiz.rounds.length !== 5 || !quiz.title.trim() || !quiz.number || !isQuizNumberValid(quiz.number) || !quiz.rounds.every(round => round.questions.length === 5)}
                     className={`px-4 py-2 rounded-full font-medium transition-colors text-sm ${
                       quiz.rounds.length === 5 && quiz.title.trim() && quiz.number && isQuizNumberValid(quiz.number) && quiz.rounds.every(round => round.questions.length === 5)
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-md w-full mx-4">
            ...
            <div className="space-y-4">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">
                ...
                <p className="mt-1">Total questions: {quiz.rounds.reduce((sum, round) => sum + round.questions.length, 0)}/25</p>
              </div>
