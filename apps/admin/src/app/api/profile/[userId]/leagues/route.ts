  ...
  try {
    ...
    const leagues = await Promise.all(
      leaderboardMemberships.map(async (membership) => {
        ...
        const leagueScores = allCompletions.map(c => (c.score / c.totalQuestions) * 100);
        ...
        const memberScores = memberUserIds.map(memberId => {
          const memberCompletions = allCompletions.filter(c => c.userId === memberId);
        ...
        const performanceOverTime = userCompletions.map(completion => {
          ...
          const sameDateCompletions = allCompletions.filter(
