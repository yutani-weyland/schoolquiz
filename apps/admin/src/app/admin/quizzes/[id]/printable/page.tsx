 ...
 * 
 * Access: /admin/quizzes/[id]/printable?pdf=1
 * 
 * This route:
...
// Helper to create color mix (like CSS color-mix)
function colorMix(
  ...
  const r = Math.round(color1.r * percent1 + color2.r * percent2)
  const g = Math.round(color1.g * percent1 + color2.g * percent2)
  const b = Math.round(color1.b * percent1 + color2.b * percent2)
  return `rgb(${r}, ${g}, ${b})`
}
...
export default async function PrintableQuizPage({
  ...
  // Get round tags (first 4 round titles)
  const roundTags = quiz.rounds
    .slice(0, 4)
    .map(r => r.title)
    .filter(Boolean)
    ...
    <div className="printable-quiz bg-white p-10 max-w-[210mm] mx-auto" style={{ fontFamily: "'Atkinson Hyperlegible', 'Inter', system-ui, sans-serif", minHeight: '100vh' }}>
      ...
      <div className="questions">
        {quiz.rounds.map((round, roundIndex) => {
          const roundColor = getRoundColor(roundIndex, round.isPeoplesRound || false)
