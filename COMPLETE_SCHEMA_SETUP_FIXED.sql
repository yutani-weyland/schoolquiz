-- Complete Database Schema Setup for SchoolQuiz (Fixed)
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'user_question_submissions' AND column_name = 'createdAt'
  ) THEN
    CREATE INDEX IF NOT EXISTS "user_question_submissions_status_createdAt_idx" 
      ON user_question_submissions(status, "createdAt");
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_question_submissions') THEN
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'run_question_stats') THEN
